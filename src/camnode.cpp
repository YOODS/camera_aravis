...
#include <sys/shm.h>
static key_t shmkey=0x41524156; //"ARAV"
...
static void stream_cb (void *user_data, ArvStreamCallbackType type, ArvBuffer *buffer)
{
		...
		if (!arv_make_thread_realtime (10)){
			...
			if (!arv_make_thread_high_priority (-10)){
				...
				printf ("Success to make stream thread high priority\n");
			}
		}else{
			printf ("Success to make stream thread realtime\n");
		}
	}
}
...
ArvGvStream *CreateStream(void)
{
	...
	unsigned int 	timeoutPacket = 1; //1 milliseconds originally
//	unsigned int 	timeoutPacket = 40;//40 milliseconds originally
//	unsigned int 	timeoutPacket = 5; //500 microseconds originally
//	unsigned int 	timeoutPacket = 3; //500 microseconds originally
...
int main(int argc, char** argv) 
{
    ...
    if (nDevices>0)
    {
        ...
        if (global.phNode->hasParam(ros::this_node::getName()+"/ID")){
            std::string idstr;
            global.phNode->getParam(ros::this_node::getName()+"/ID", idstr);
            for(int i=0;i<idstr.length();i++) shmkey+=(i+1)*idstr.at(i);
        }
